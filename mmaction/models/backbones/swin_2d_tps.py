...
class WindowAttention3D(BaseModule):
    ...
    def forward(self,
        ...
        if self.shift:
            # B*nW, num_head, Wd*Wh*Ww, C/num_head
        ...
        attn = attn.view(batch_size,frame_len,-1,self.num_heads,N,N).permute(0,2,1,3,4,5) 
        + relative_position_bias.unsqueeze(0).unsqueeze(1) # B_, nH, N, N
...
class SwinTransformer2D_TPS(BaseModule):
    ...
    def forward(self, x: torch.Tensor) -> \
        ...
        """Forward function for Swin2d-tps Transformer."""
...
class PatchShift(nn.Module):
    def __init__(self, n_div=8, inv=False, ratio=1):
        ...
        if inv:
            ...
            print(f'=> Using bayershift, head_num: {self.fold_div}, ratio {ratio}, tps')
