...
class ViT_CLIP_TPS(nn.Module):
    ...
    def _freeze_stages(self) -> None:
        ## freeze some parameters
        for name, param in self.named_parameters():
            if 'temporal_embedding' not in name and 'ln_post' not in name and 'cls_head' not in name and 'Adapter' not in name and 'dpe' not in name and 'shift_conv' not in name :
                param.requires_grad = False
