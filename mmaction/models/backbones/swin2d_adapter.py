...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, num_ttokens, window_size, num_heads, use_temporal=True, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
    ...
    def forward(self, x, mask=None, temporal=False):
        """
        Args:
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_frames, num_heads, window_size=7, shift_size=0,
        ...
        self.attn = WindowAttention(
            dim, num_ttokens=num_frames, window_size=to_2tuple(self.window_size), num_heads=num_heads, use_temporal=t_attn,
    ...
    def forward(self, x):
        ...
        if self.t_attn:
            ...
            res_temporal = self.attn(self.norm1(x), temporal=True)
...
class SwinTransformer2D_Adapter(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, pretrained=None, img_size=224, patch_size=4, num_frames=32, in_chans=3, num_classes=1000,
        ...
        self.patches_resolution = patches_resolution
        self.num_Ttokens = num_frames // patch_size[0]
    ...
    def forward_features(self, x):
        x, B, T = self.patch_embed(x)  # x in shape (BT, HW, C)
        if self.ape:
        ...
        H = self.layers[-1].input_resolution[0]
    ...
    def train(self, mode=True):
        ...
        super(SwinTransformer2D_Adapter, self).train(mode)
        self._freeze_stages()
